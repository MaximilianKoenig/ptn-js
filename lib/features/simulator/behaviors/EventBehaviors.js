import { getEventDefinition, isTypedEvent } from "../../../util/ElementHelper";

import { ScopeTraits } from "../ScopeTraits";

import { isEventSubProcess, isLinkCatch } from "../util/ModelUtil";
...
EventBehaviors.$inject = ["simulator", "elementRegistry", "scopeBehavior"];
...
EventBehaviors.prototype.get = function (element) {
  const behaviors = {
    "bpmn:LinkEventDefinition": (context) => {
      ...
      const linkTargets = parentElement.children.filter(
        (element) =>
          isLinkCatch(element) && getLinkDefinition(element).name === link.name,
      );
  ...
  const entry = Object.entries(behaviors).find((entry) =>
    isTypedEvent(element, entry[0]),
  );
