...
import { filterSet, findSet } from "./util/SetUtil";

import { eventsMatch } from "./util/EventsUtil";
...
export default function Simulator(injector, eventBus, elementRegistry) {
  ...
  function enter(context) {
    const { element, scope: parentScope, initiator = parentScope } = context;
  ...
  function exit(context) {
    const { element, scope, initiator = scope } = context;
  ...
  function trigger(context) {
    const { event: _event, initiator, scope } = context;
    ...
    const matchingSubscriptions = filterSet(subscriptions, (subscription) =>
      eventsMatch(event, subscription.event),
    );
    ...
    if (!interrupting.length) {
      return nonInterrupting
        .map((subscription) => subscription.triggerFn(initiator))
        .flat();
    }

    const interrupt =
      interrupting.find((subscription) => !subscription.event.boundary) ||
      interrupting[0];

    const remainingSubscriptions = filterSet(
      subscriptions,
      (subscription) =>
        subscription.event.persistent ||
        isRethrow(subscription.event, interrupt.event),
    );
  ...
  function unsubscribe(subscription) {
    const { scope, event } = subscription;
  ...
  function getEvent(element) {
    ...
    if (is(eventDefinition, "bpmn:CompensateEventDefinition")) {
      let ref =
        eventDefinition.activityRef &&
        elementRegistry.get(eventDefinition.activityRef.id);
  ...
  function createScope(context, emitEvent = true) {
    const { element, parent: parentScope, initiator } = context;
  ...
  function subscriptionFilter(filter) {
    ...
    const { event: _event, element, scope } = filter;
  ...
  function scopeSubscriptionFilter(event) {
    const matchesSubscription =
      event === "function" ? event : subscriptionFilter(event);
  ...
  function scopeFilter(filter) {
    ...
    const isSubscribed = subscribedTo
      ? scopeSubscriptionFilter(subscribedTo)
      : () => true;
  ...
  function initializeRootScopes() {
    ...
    elementRegistry.forEach((element) => {
      ...
      for (const startEvent of startEvents) {
        ...
        subscribe(scope, event, (initiator) =>
          signal({
            element,
  ...
  function initializeScope(context) {
    ...
    const { children = [], attachers = [] } = element;
...
Simulator.$inject = ["injector", "eventBus", "elementRegistry"];
...
function isRethrow(event, interrupt) {
  return event.type === interrupt.type && event.boundary && !interrupt.boundary;
}

function isImplicitMessageCatch(element) {
  return (
    is(element, "bpmn:ReceiveTask") ||
    element.incoming.some((element) => is(element, "bpmn:MessageFlow"))
  );
}
