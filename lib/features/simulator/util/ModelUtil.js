...
export function isMessageCatch(element) {
  return (
    isCatchEvent(element) &&
    isTypedEvent(element, "bpmn:MessageEventDefinition")
  );
}

export function isLinkCatch(element) {
  return (
    isCatchEvent(element) && isTypedEvent(element, "bpmn:LinkEventDefinition")
  );
}
...
export function isCompensationEvent(element) {
  return (
    isCatchEvent(element) &&
    isTypedEvent(element, "bpmn:CompensateEventDefinition")
  );
}

export function isCompensationActivity(element) {
  return (
    is(element, "bpmn:Activity") && element.businessObject.isForCompensation
  );
}

export function isCatchEvent(element) {
  return (
    (is(element, "bpmn:CatchEvent") || is(element, "bpmn:ReceiveTask")) &&
    !isLabel(element)
  );
}
...
export function isInterrupting(element) {
  return (
    (is(element, "bpmn:StartEvent") &&
      getBusinessObject(element).isInterrupting) ||
    (is(element, "bpmn:BoundaryEvent") &&
      getBusinessObject(element).cancelActivity)
  );
}
