...
export default function ContextPads(
  ...
  this.registerHandler("ptn:Transition", TriggerHandler);
...
ContextPads.prototype._updateElementContextPads = function (element, handler) {
  const contextPads = (handler.createContextPads(element) || []).filter(
    (p) => p,
  );
  ...
  for (const contextPad of contextPads) {
    ...
    let existingOverlay = existingOverlays.find((o) => o.hash === hash);
    ...
    if (_contexts) {
      ...
      const shownScopes = contexts.filter((c) =>
        this._scopeFilter.isShown(c.scope),
      );
