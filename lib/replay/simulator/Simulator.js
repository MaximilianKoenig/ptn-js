...
// ToDo: Think of a way to avoid manipulating "initialMarking" in the simulation
  ...
  function getCurrentMarking() {
    ...
    elementRegistry.forEach((element) => {
      if (element.type === "ptn:Place") {
        marking.set(
          element.id,
          element.businessObject.initialMarking ? element.businessObject.initialMarking : 0,
        );
      }
    });
    return marking;
    ...
    if (canTrigger(element, flowDirectionInverted)) {
      ...
      if (incoming) {
        for (const i of incoming) {
          ...
          const marking = source.businessObject.initialMarking ?? 0;
      ...
      if (outgoing) {
        for (const o of outgoing) {
          ...
          const marking = target.businessObject.initialMarking ?? 0;
