...
export default class ElementFactory extends BaseElementFactory{
  ...
  createElement(elementType, attrs) {
    ...
    if (!businessObject.di) {
      if (elementType === 'root') {
        businessObject.di = this._customElementFactory.createDiPlane(businessObject, [], {
          id: businessObject.id + '_di'
        });
      } else if (elementType === 'connection') {
        businessObject.di = this._customElementFactory.createDiEdge(businessObject, [], {
          id: businessObject.id + '_di'
        });
      } else {
        businessObject.di = this._customElementFactory.createDiShape(businessObject, [], {
          id: businessObject.id + '_di'
        });
      }
    }
