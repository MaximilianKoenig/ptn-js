...
// CustomModelerTodo: Insert moddle prefix.
const MODDLE_PREFIX = 'ptn';
...
export default class CustomModdle extends Moddle {
   ...
   *
   * @param {String}   xmlStr
   * @param {String}   [typeName='MODDLE_PREFIX:Definitions'] name of the root element
  ...
  fromXML(xmlStr, typeName, options) {
    if (!isString(typeName)) {
      options = typeName;
      typeName = `${MODDLE_PREFIX}:Definitions`;
    }
   ...
   *
   * @param {String}   element    the root element, typically an instance of `${MODDLE_PREFIX}:Definitions`
